...
const Movie = () => {
  ...
  const [filteredMoviesByName, setFilteredMoviesByName] = useState(null);
  ...
  const filterMoviesByProducer = (id, movies) => {
    if (id) {
      setFilteredMovies(movies?.filter((m) => m.ProducerId === id));
    } else {
      setFilteredMovies(movies); // Display all movies
    }
  };
